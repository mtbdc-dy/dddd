...
/**
  *企业所得税和非居民企业所得税(包括季报和年报)--企业基础信息表（表三）--A000000
 */
public class A000000_Qyxxjcb3Handler implements GSModuleBase<GSSiteHandler> {
	...
	public SiteStatus start(GSSiteHandler siteHandler) throws SiteLoginFailedException {
		...
		new CompanyDataDaoImpl().addData(siteHandler.params.getCompanyId(), "10053", list);
	...
	private Map<String, Object> parseLssb(Element tr) {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("10053001", StringUtils.StringFormat(tr.child(2).child(0).text()));
		map.put("10053002", StringUtils.StringFormat(tr.child(3).child(0).text()));
		map.put("10053003", StringUtils.StringFormat(tr.child(4).child(0).text()));
		map.put("serialNumber", "10053");
		return map;
	}
