...
/**
 * 获取解析存储申报信息 -- 固定资产进项税额抵扣情况表
 */
public class GdzcjxsedkqkHandler implements GSModuleBase<GSSiteHandler> {
...
//	CompanyDataDao companyDataDao;
	...
	public SiteStatus start(GSSiteHandler siteHandler) throws SiteLoginFailedException {
		...
		for(int i=1;i<trElements.size();i++){
			...
			//http://100.0.0.1:8001/ctais2/wssb/sjcx/print_zzs_flzl5.jsp?k=40
			...
			boolean flag = true;
			for(Element b:aDyxm){
				if(b.attr("href").startsWith("print_zzs_flzl5.jsp")){
		...
		new CompanyDataDaoImpl().addData(siteHandler.params.getCompanyId(), "10013", list);
	...
	private Map<String,Object> parseLssb(Element tr){
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("10013001", StringUtils.StringFormat(tr.child(2).child(0).text()));
		map.put("10013002", StringUtils.StringFormat(tr.child(3).child(0).text()));
		map.put("10013003", StringUtils.StringFormat(tr.child(4).child(0).text()));
		map.put("serialNumber", "10013");
		return map;
	}
	
	/**
	 * 解析增值税一般纳税人-- 固定资产进项税额抵扣情况表
	 * @param html
	 * @param href_
	 * @return
	 */
