...
/**
 * 获取解析存储申报信息 -- 不动产分期抵扣计算表(附表5)
 */
public class BdcfqdkHandler implements GSModuleBase<GSSiteHandler> {
	...
	public SiteStatus start(GSSiteHandler siteHandler) throws SiteLoginFailedException {
		...
		for (int i = 1; i < trElements.size(); i++) {
			...
			//http://100.0.0.1:8001/ctais2/wssb/sjcx/sb_zzs_bdcfqdk_201605.jsp?jspLogic=print&k=55
			...
			boolean flag = true;
			for (Element b : aDyxm) {
				if (b.attr("href").startsWith("sb_zzs_bdcfqdk_201605")) {
		...
		new CompanyDataDaoImpl().addData(siteHandler.params.getCompanyId(), "10019", list);
	...
	private Map<String, Object> parseLssb(Element tr) {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("10019001", StringUtils.StringFormat(tr.child(2).child(0).text()));
		map.put("10019002", StringUtils.StringFormat(tr.child(3).child(0).text()));
		map.put("10019003", StringUtils.StringFormat(tr.child(4).child(0).text()));
		map.put("serialNumber", "10019");
		return map;
	}

	/**
	 * 解析增值税一般纳税人-- 不动产分期抵扣计算表(附表5)
	 * 
	 * @param html
	 * @param href_
	 * @return
		...
		for (int k = 2; k < personTrs1.size(); k++) {
			...
			index6 = index4 + "001";
				...
				for (int k1 = 0; k1 < tds1.size(); k1++) {
					String td = StringUtils.StringFormat(tds1.get(k1).getElementsByTag("input").val());
					map.put(index6,td);
