...
/**
  * 企业所得税和非居民企业所得税(包括季报和年报)--附表十--资产减值准备项目调整明细表
 */
public class Fb10_zcjztzmxbHandler implements GSModuleBase<GSSiteHandler> {
	...
	public SiteStatus start(GSSiteHandler siteHandler) throws SiteLoginFailedException {
		...
		for (int i =1; i < trElements.size(); i++) {
			...
			//http://100.0.0.1:8001/ctais2/wssb/sjcx/print_ndsds_fb10.jsp?k=2
			...
			boolean flag = true;
			for (Element b : aDyxm) {
				if (b.attr("href").startsWith("print_ndsds_fb10.jsp")) {
		...
		new CompanyDataDaoImpl().addData(siteHandler.params.getCompanyId(), "10046", list);
	...
	private Map<String, Object> parseLssb(Element tr) {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("10046001", StringUtils.StringFormat(tr.child(2).child(0).text()));
		map.put("10046002", StringUtils.StringFormat(tr.child(3).child(0).text()));
		map.put("10046003", StringUtils.StringFormat(tr.child(4).child(0).text()));
		map.put("serialNumber", "10046");
		return map;
	}
