...
/**
  * 企业所得税和非居民企业所得税(包括季报和年报)--内资企业所得税(季度A类)--附表一(不征税收入和税基类减免应纳税所得额明细表 )
 */
public class Bzssjlmxb_fb1Handler implements GSModuleBase<GSSiteHandler> {
	...
	public SiteStatus start(GSSiteHandler siteHandler) throws SiteLoginFailedException {
		...
		for (int i =1; i < trElements.size(); i++) {
			...
			//http://100.0.0.1:8001/ctais2/wssb/sjcx/print_jdsds_bzssjl_mxb.jsp?k=18
			...
			boolean flag = true;
			for (Element b : aDyxm) {
				if (b.attr("href").startsWith("print_jdsds_bzssjl_mxb.jsp")) {
		...
		new CompanyDataDaoImpl().addData(siteHandler.params.getCompanyId(), "10048", list);
	...
	private Map<String, Object> parseLssb(Element tr) {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("10048001", StringUtils.StringFormat(tr.child(2).child(0).text()));
		map.put("10048002", StringUtils.StringFormat(tr.child(3).child(0).text()));
		map.put("10048003", StringUtils.StringFormat(tr.child(4).child(0).text()));
		map.put("serialNumber", "10048");
		return map;
	}
