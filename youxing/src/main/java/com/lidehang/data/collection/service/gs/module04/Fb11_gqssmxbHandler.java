...
/**
  * 企业所得税和非居民企业所得税(包括季报和年报)--附表十一--长期股权投资所得（损失）明细表
 */
public class Fb11_gqssmxbHandler implements GSModuleBase<GSSiteHandler> {
	...
	public SiteStatus start(GSSiteHandler siteHandler) throws SiteLoginFailedException {
		...
		for (int i =1; i < trElements.size(); i++) {
			...
			//http://100.0.0.1:8001/ctais2/wssb/sjcx/print_ndsds_fb11.jsp?k=2
			...
			boolean flag = true;
			for (Element b : aDyxm) {
				if (b.attr("href").startsWith("print_ndsds_fb11.jsp")) {
		...
		new CompanyDataDaoImpl().addData(siteHandler.params.getCompanyId(), "10047", list);
	...
	private Map<String, Object> parseLssb(Element tr) {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("10047001", StringUtils.StringFormat(tr.child(2).child(0).text()));
		map.put("10047002", StringUtils.StringFormat(tr.child(3).child(0).text()));
		map.put("10047003", StringUtils.StringFormat(tr.child(4).child(0).text()));
		map.put("serialNumber", "10047");
		return map;
	}
