...
/**
 * 企业所得税年度关联企业申报表-- 无形资产表（表五)
 */
public class Wxzcb_b5Handler implements GSModuleBase<GSSiteHandler> {
	...
	public SiteStatus start(GSSiteHandler siteHandler) throws SiteLoginFailedException {
		...
		for (int i = 1; i < trElements.size(); i++) {
			...
			boolean flag = true;
			for (Element b : aDyxm) {
				//http://100.0.0.1:8001/ctais2/wssb/sjcx/print_ndywwl_b5.jsp?k=0
				if (b.attr("href").startsWith("print_ndywwl_b5")) {
		...
		new CompanyDataDaoImpl().addData(siteHandler.params.getCompanyId(), "10031", list);
	...
	private Map<String, Object> parseLssb(Element tr) {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("10031001", StringUtils.StringFormat(tr.child(2).child(0).text()));
		map.put("10031002", StringUtils.StringFormat(tr.child(3).child(0).text()));
		map.put("10031003", StringUtils.StringFormat(tr.child(4).child(0).text()));
		map.put("serialNumber", "10031");
		return map;
	}
	...
	private Map<String, Object> parseSBB(String html) {
		...
		String index4 = "10031004";
		String index6 = "";
		Elements personTrs1 = tables.get(1).getElementsByTag("tr");
		for (int i = 4; i < personTrs1.size(); i++) {
			Map<String, Object> map = new HashMap<String, Object>();
