...
/**
 *企业所得税和非居民企业所得税(包括季报和年报)--附表四——企业所得税弥补亏损明细表
 */
public class Fb4_qymbksmxbHandler implements GSModuleBase<GSSiteHandler> {
	...
	public SiteStatus start(GSSiteHandler siteHandler) throws SiteLoginFailedException {
		...
		for (int i =1; i < trElements.size(); i++) {
			...
			//http://100.0.0.1:8001/ctais2/wssb/sjcx/print_ndsds_fb4.jsp?k=7
			...
			boolean flag = true;
			for (Element b : aDyxm) {
				if (b.attr("href").startsWith("print_ndsds_fb4.jsp")) {
		...
		new CompanyDataDaoImpl().addData(siteHandler.params.getCompanyId(), "10040", list);
	...
	private Map<String, Object> parseLssb(Element tr) {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("10040001", StringUtils.StringFormat(tr.child(2).child(0).text()));
		map.put("10040002", StringUtils.StringFormat(tr.child(3).child(0).text()));
		map.put("10040003", StringUtils.StringFormat(tr.child(4).child(0).text()));
		map.put("serialNumber", "10040");
		return map;
	}
	...
	private Map<String, Object> parseSBB(String html) {
		...
		String index4 = "10040004";
		String index6 = "";
		Elements personTrs1 = tables.get(1).getElementsByTag("tr");
		for (int k =3; k < personTrs1.size(); k++) {
			Map<String, Object> map = new HashMap<String, Object>();
			...
			for (int i = 1; i < tds1.size(); i++) {
				...
				if(k==personTrs1.size()-1){
					String td = StringUtils.StringFormat(tds1.get(i).text());
					map.put(index6,td);
					index6 = String.valueOf((Long.parseLong(index6) + 6));//项目
				}else {
					String td = StringUtils.StringFormat(tds1.get(i).text());
					map.put(index6,td);
					index6 = String.valueOf((Long.parseLong(index6) + 1));//项目
				}
