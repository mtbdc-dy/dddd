 ...
 * 获取解析存储征收信息---个人利息所得税
 */
public class ZsgrlxsdsHandler implements GSModuleBase<GSSiteHandler> {
...
//	CompanyDataDao companyDataDao;
	...
	public SiteStatus start(GSSiteHandler siteHandler) throws SiteLoginFailedException {
		...
		//获取增值税页面数据
		String zzsListHtml = siteHandler.getPage("http://100.0.0.1:8001/ctais2/wssb/sjcx/sbtj_yzscx.jsp?sssq_q="+siteHandler.params.getStartTimeStr()+"&sssq_z="+siteHandler.params.getEndTimeStr()+"&zsxm_dm=06");
//		System.out.println(zzsListHtml);
		...
		new CompanyDataDaoImpl().addData(siteHandler.params.getCompanyId(), "10011", list);
		return SiteStatus.success;
	}
	
	/**
	 * 解析征收信息个人利息所得税历史申报列表中的某一项
	 */
	private Map<String,Object> parseLssb(Element tr){
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("10011001", StringUtils.StringFormat(tr.child(2).child(0).text()));
		map.put("10011002", StringUtils.StringFormat(tr.child(3).child(0).text()));
		map.put("10011003", StringUtils.StringFormat(tr.child(4).child(0).text()));
		map.put("10011004", StringUtils.StringFormat(tr.child(5).child(0).text()));
		map.put("serialNumber", "10011");
		return map;
	}
