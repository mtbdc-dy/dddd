...
/**
 *企业所得税和非居民企业所得税(包括季报和年报)--附表六——境外所得税抵免计算明细表
 */
public class Fb6_jwdmmxbHandler implements GSModuleBase<GSSiteHandler> {
	...
	public SiteStatus start(GSSiteHandler siteHandler) throws SiteLoginFailedException {
		...
		for (int i =1; i < trElements.size(); i++) {
			...
			//http://100.0.0.1:8001/ctais2/wssb/sjcx/print_ndsds_fb6.jsp?k=7
			...
			boolean flag = true;
			for (Element b : aDyxm) {
				if (b.attr("href").startsWith("print_ndsds_fb6.jsp")) {
		...
		new CompanyDataDaoImpl().addData(siteHandler.params.getCompanyId(), "10042", list);
	...
	private Map<String, Object> parseLssb(Element tr) {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("10042001", StringUtils.StringFormat(tr.child(2).child(0).text()));
		map.put("10042002", StringUtils.StringFormat(tr.child(3).child(0).text()));
		map.put("10042003", StringUtils.StringFormat(tr.child(4).child(0).text()));
		map.put("serialNumber", "10042");
		return map;
	}
	...
	private Map<String, Object> parseSBB(String html) {
		...
		String index4 = "10042004";
		String index6 = "";
		String td1="";
		Elements personTrs1 = tables.get(1).getElementsByTag("tr");
		for (int k =3; k < personTrs1.size()-1; k++) {
			Map<String, Object> map = new HashMap<String, Object>();
			Elements tds1 = personTrs1.get(k).getElementsByTag("td");
			index6 = index4 + "001";
			if("".equals(StringUtils.StringFormat(tds1.get(0).text()))){
				continue;
			}
			if(tds1.size()==18){
				td1=StringUtils.StringFormat(tds1.get(0).text());
