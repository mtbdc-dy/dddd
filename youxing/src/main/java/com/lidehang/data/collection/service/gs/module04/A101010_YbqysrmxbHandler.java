...
/**
  * 企业所得税和非居民企业所得税(包括季报和年报)--一般企业收入明细表--A101010
 */
public class A101010_YbqysrmxbHandler implements GSModuleBase<GSSiteHandler> {
	...
	public SiteStatus start(GSSiteHandler siteHandler) throws SiteLoginFailedException {
		...
		for (int i =1; i < trElements.size(); i++) {
			...
			//http://100.0.0.1:8001/ctais2/wssb/sjcx/print_ndsds_2014_sr_yb.jsp?k=17
			...
			boolean flag = true;
			for (Element b : aDyxm) {
				if (b.attr("href").startsWith("print_ndsds_2014_sr_yb.jsp")) {
		...
		new CompanyDataDaoImpl().addData(siteHandler.params.getCompanyId(), "10055", list);
	...
	private Map<String, Object> parseLssb(Element tr) {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("10055001", StringUtils.StringFormat(tr.child(2).child(0).text()));
		map.put("10055002", StringUtils.StringFormat(tr.child(3).child(0).text()));
		map.put("10055003", StringUtils.StringFormat(tr.child(4).child(0).text()));
		map.put("serialNumber", "10055");
		return map;
	}
