...
/**
 * 获取解析存储申报信息 --企业所得税资产损失税前扣除申报表-- 资产损失清单申报明细表
 */
public class Zcssqd_sbmxHandler implements GSModuleBase<GSSiteHandler> {
	...
	public SiteStatus start(GSSiteHandler siteHandler) throws SiteLoginFailedException {
		...
		for (int i = 1; i < trElements.size(); i++) {
			...
			boolean flag = true;
			for (Element b : aDyxm) {
				//http://100.0.0.1:8001/ctais2/wssb/sjcx/print_ndzcss_b1.jsp?k=1
				if (b.attr("href").startsWith("print_ndzcss_b1")) {
		...
		new CompanyDataDaoImpl().addData(siteHandler.params.getCompanyId(), "10025", list);
	...
	private Map<String, Object> parseLssb(Element tr) {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("10025001", StringUtils.StringFormat(tr.child(2).child(0).text()));
		map.put("10025002", StringUtils.StringFormat(tr.child(3).child(0).text()));
		map.put("10025003", StringUtils.StringFormat(tr.child(4).child(0).text()));
		map.put("serialNumber", "10025");
		return map;
	}

	/**
	 * 解析增值税申报表--企业所得税资产损失税前扣除申报表-- 资产损失清单申报明细表
	 */
