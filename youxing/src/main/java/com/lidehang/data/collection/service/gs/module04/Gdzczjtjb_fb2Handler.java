...
/**
  * 企业所得税和非居民企业所得税(包括季报和年报)--内资企业所得税(季度A类)--附表二(固定资产加速折旧(扣除)预缴情况统计表)
 */
public class Gdzczjtjb_fb2Handler implements GSModuleBase<GSSiteHandler> {
	...
	public SiteStatus start(GSSiteHandler siteHandler) throws SiteLoginFailedException {
		...
		for (int i =1; i < trElements.size(); i++) {
			...
			//http://100.0.0.1:8001/ctais2/wssb/sjcx/print_jdsds_2015_gdzc_zjyj.jsp?k=25
			...
			boolean flag = true;
			for (Element b : aDyxm) {
				if (b.attr("href").startsWith("print_jdsds_2015_gdzc_zjyj.jsp")) {
		...
		new CompanyDataDaoImpl().addData(siteHandler.params.getCompanyId(), "10050", list);
	...
	private Map<String, Object> parseLssb(Element tr) {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("10050001", StringUtils.StringFormat(tr.child(2).child(0).text()));
		map.put("10050002", StringUtils.StringFormat(tr.child(3).child(0).text()));
		map.put("10050003", StringUtils.StringFormat(tr.child(4).child(0).text()));
		map.put("serialNumber", "10050");
		return map;
	}
	...
	private Map<String, Object> parseSBB(String html) {
		...
		String index4 = "10050004";
		String index6 = "";
		String td1="";
		Elements personTrs1 = tables.get(3).getElementsByTag("tr");
		for (int k =4; k < personTrs1.size(); k++) {
			String[] arr=null;
			arr=StringUtils.getJsNum1(html, k-3, "tr");
			Map<String, Object> map = new HashMap<String, Object>();
			Elements tds1 = personTrs1.get(k).getElementsByTag("td");
			index6 = index4 + "001";
			if (tds1.size()==20) {
				for (int i = 2; i < tds1.size(); i++) {
				...
				for (int i =1; i < tds1.size(); i++) {
					...
					if(i==1||td.equals("*")){
						...
						map.put(index6,StringUtils.remSe(arr[i-1]));
					}
					index6 = String.valueOf((Long.parseLong(index6) + 1));
