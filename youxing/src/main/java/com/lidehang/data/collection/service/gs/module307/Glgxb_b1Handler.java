...
/**
 * 企业所得税年度关联企业申报表-- 关联关系表（表一）
 */
public class Glgxb_b1Handler implements GSModuleBase<GSSiteHandler> {
	...
	public SiteStatus start(GSSiteHandler siteHandler) throws SiteLoginFailedException {
		...
		for (int i = 1; i < trElements.size(); i++) {
			...
			boolean flag = true;
			for (Element b : aDyxm) {
				//http://100.0.0.1:8001/ctais2/wssb/sjcx/print_ndywwl_b1.jsp?k=2
				if (b.attr("href").startsWith("print_ndywwl_b1")) {
		...
		new CompanyDataDaoImpl().addData(siteHandler.params.getCompanyId(), "10027", list);
	...
	private Map<String, Object> parseLssb(Element tr) {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("10027001", StringUtils.StringFormat(tr.child(2).child(0).text()));
		map.put("10027002", StringUtils.StringFormat(tr.child(3).child(0).text()));
		map.put("10027003", StringUtils.StringFormat(tr.child(4).child(0).text()));
		map.put("serialNumber", "10027");
		return map;
	}
