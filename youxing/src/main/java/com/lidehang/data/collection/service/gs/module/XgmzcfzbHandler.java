...
/**
 * 获取解析存储申报信息 --增值税小规模纳税人-- 资产负债表
 */
public class XgmzcfzbHandler implements GSModuleBase<GSSiteHandler> {
	...
	public SiteStatus start(GSSiteHandler siteHandler) throws SiteLoginFailedException {
		...
		for (int i = 1; i < trElements.size(); i++) {
			...
			boolean flag = true;
			for (Element b : aDyxm) {
				// http://100.0.0.1:8001/ctais2/wssb/sjcx/print_zcfzb1.jsp?k=1
				if (b.attr("href").startsWith("print_zcfzb1")) {
		...
		new CompanyDataDaoImpl().addData(siteHandler.params.getCompanyId(), "10023", list);
	...
	private Map<String, Object> parseLssb(Element tr) {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("10023001", StringUtils.StringFormat(tr.child(2).child(0).text()));
		map.put("10023002", StringUtils.StringFormat(tr.child(3).child(0).text()));
		map.put("10023003", StringUtils.StringFormat(tr.child(4).child(0).text()));
		map.put("serialNumber", "10023");
		return map;
	}

	/**
	 * 解析增值税申报表--增值税小规模纳税人-- 资产负债表
	 */
