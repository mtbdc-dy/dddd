 ...
 * 获取解析存储增征收信息---消费税
 */
public class ZsxfsHandler implements GSModuleBase<GSSiteHandler> {
...
//	CompanyDataDao companyDataDao;
	...
	public SiteStatus start(GSSiteHandler siteHandler) throws SiteLoginFailedException {
		...
		//获取增值税页面数据
		String zzsListHtml = siteHandler.getPage("http://100.0.0.1:8001/ctais2/wssb/sjcx/sbtj_yzscx.jsp?sssq_q="+siteHandler.params.getStartTimeStr()+"&sssq_z="+siteHandler.params.getEndTimeStr()+"&zsxm_dm=03");
//		System.out.println(zzsListHtml);
		...
		new CompanyDataDaoImpl().addData(siteHandler.params.getCompanyId(), "10008", list);
		return SiteStatus.success;
	}
	
	/**
	 * 解析征收信息---消费税 历史申报列表中的某一项
	 */
	private Map<String,Object> parseLssb(Element tr){
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("10008001", StringUtils.StringFormat(tr.child(2).child(0).text()));
		map.put("10008002", StringUtils.StringFormat(tr.child(3).child(0).text()));
		map.put("10008003", StringUtils.StringFormat(tr.child(4).child(0).text()));
		map.put("10008004", StringUtils.StringFormat(tr.child(5).child(0).text()));
		map.put("serialNumber", "10008");
		return map;
	}
