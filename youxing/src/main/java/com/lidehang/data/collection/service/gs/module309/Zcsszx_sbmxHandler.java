package com.lidehang.data.collection.service.gs.module309;
...
/**
 * 获取解析存储申报信息 --企业所得税资产损失税前扣除申报表-- 资产损失专项申报明细表
 */
public class Zcsszx_sbmxHandler implements GSModuleBase<GSSiteHandler> {
	...
	public SiteStatus start(GSSiteHandler siteHandler) throws SiteLoginFailedException {
		...
		String zzsListHtml = siteHandler.getPage(
				"http://100.0.0.1:8001/ctais2/wssb/sjcx/sbtj_ysbcx.jsp?sssq_q=" + siteHandler.params.getStartTimeStr()
						+ "&sssq_z=" + siteHandler.params.getEndTimeStr() + "&zsxm_dm=309");
		...
		for (int i = 1; i < trElements.size(); i++) {
			...
			boolean flag = true;
			for (Element b : aDyxm) {
				//http://100.0.0.1:8001/ctais2/wssb/sjcx/print_ndzcss_b2.jsp?k=1
				if (b.attr("href").startsWith("print_ndzcss_b2")) {
		...
		new CompanyDataDaoImpl().addData(siteHandler.params.getCompanyId(), "10026", list);
	...
	private Map<String, Object> parseLssb(Element tr) {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("10026001", StringUtils.StringFormat(tr.child(2).child(0).text()));
		map.put("10026002", StringUtils.StringFormat(tr.child(3).child(0).text()));
		map.put("10026003", StringUtils.StringFormat(tr.child(4).child(0).text()));
		map.put("serialNumber", "10026");
		return map;
	}

	/**
	 * 解析增值税申报表--企业所得税资产损失税前扣除申报表-- 资产损失专项申报明细表
	 */
