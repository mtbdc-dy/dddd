...
/**
 *企业所得税和非居民企业所得税(包括季报和年报)--固定资产加速折旧、扣除明细表 --A105081
 */
public class A105081_GdzczjmxbHandler implements GSModuleBase<GSSiteHandler> {
	...
	public SiteStatus start(GSSiteHandler siteHandler) throws SiteLoginFailedException {
		...
		for (int i =1; i < trElements.size(); i++) {
			...
			//http://100.0.0.1:8001/ctais2/wssb/sjcx/print_ndsds_2014_nstz_zjtx_js.jsp?k=17
			...
			boolean flag = true;
			for (Element b : aDyxm) {
				String bhref=b.attr("href");
				if(bhref.indexOf("?")==-1){
					String ddd=StringUtils.ampformat(bhref);
					 bhref=ddd;
				}
				if (bhref.startsWith("print_ndsds_2014_nstz_zjtx_js.jsp")) {
					String response2 = siteHandler.getPage("http://100.0.0.1:8001/ctais2/wssb/sjcx/" + bhref);
		...
		new CompanyDataDaoImpl().addData(siteHandler.params.getCompanyId(), "10061", list);
	...
	private Map<String, Object> parseLssb(Element tr) {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("10061001", StringUtils.StringFormat(tr.child(2).child(0).text()));
		map.put("10061002", StringUtils.StringFormat(tr.child(3).child(0).text()));
		map.put("10061003", StringUtils.StringFormat(tr.child(4).child(0).text()));
		map.put("serialNumber", "10061");
		return map;
	}
