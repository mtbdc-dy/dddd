...
/**
  *企业所得税和非居民企业所得税(包括季报和年报)--期间费用明细表--A104000
 */
public class A104000_QjfymxbHandler implements GSModuleBase<GSSiteHandler> {
	...
	public SiteStatus start(GSSiteHandler siteHandler) throws SiteLoginFailedException {
		...
		for (int i =1; i < trElements.size(); i++) {
			...
			//http://100.0.0.1:8001/ctais2/wssb/sjcx/print_ndsds_2014_qjfy.jsp?k=17
			...
			boolean flag = true;
			for (Element b : aDyxm) {
				if (b.attr("href").startsWith("print_ndsds_2014_qjfy.jsp")) {
		...
		new CompanyDataDaoImpl().addData(siteHandler.params.getCompanyId(), "10057", list);
	...
	private Map<String, Object> parseLssb(Element tr) {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("10057001", StringUtils.StringFormat(tr.child(2).child(0).text()));
		map.put("10057002", StringUtils.StringFormat(tr.child(3).child(0).text()));
		map.put("10057003", StringUtils.StringFormat(tr.child(4).child(0).text()));
		map.put("serialNumber", "10057");
		return map;
	}
	/**
	 *企业所得税和非居民企业所得税(包括季报和年报)--期间费用明细表--A104000
			...
			index6 = index4 + "001";
				for (int i = 1; i < tds1.size(); i++) {
					...
					if(i==1||td.equals("*")){
						...
						String aa= StringUtils.remSe(arr[k-2]);
						map.put(index6,aa);
					}
