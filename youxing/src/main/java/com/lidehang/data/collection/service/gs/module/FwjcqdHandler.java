 ...
 * 获取解析存储申报信息 -- 服务减除清单
 */
public class FwjcqdHandler implements GSModuleBase<GSSiteHandler> {
	...
	public SiteStatus start(GSSiteHandler siteHandler) throws SiteLoginFailedException {
		...
		for (int i = 1; i < trElements.size(); i++) {
			...
			//http://100.0.0.1:8001/ctais2/wssb/sjcx/print_zzs_flzl6_2013.jsp?k=41
			...
			boolean flag = true;
			for (Element b : aDyxm) {
				if (b.attr("href").startsWith("print_zzs_flzl6_2013")) {
		...
		new CompanyDataDaoImpl().addData(siteHandler.params.getCompanyId(), "10021", list);
	...
	private Map<String, Object> parseLssb(Element tr) {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("10021001", StringUtils.StringFormat(tr.child(2).child(0).text()));
		map.put("10021002", StringUtils.StringFormat(tr.child(3).child(0).text()));
		map.put("10021003", StringUtils.StringFormat(tr.child(4).child(0).text()));
		map.put("serialNumber", "10021");
		return map;
	}
			...
			Elements tds1 = personTrs1.get(k).getElementsByTag("td");
				...
				for (int k1 = 0; k1 < tds1.size(); k1++) {
					...
					if(k==(personTrs1.size()-1)){
						index6 = String.valueOf((Long.parseLong(index6) + 7));
					}else{
