...
/**
 *企业所得税和非居民企业所得税(包括季报和年报)--企业所得税弥补亏损明细表 --A106000
 */
public class A106000_QysdsmbmsbHandler implements GSModuleBase<GSSiteHandler> {
	...
	public SiteStatus start(GSSiteHandler siteHandler) throws SiteLoginFailedException {
		...
		for (int i =1; i < trElements.size(); i++) {
			...
			//http://100.0.0.1:8001/ctais2/wssb/sjcx/print_ndsds_2014_mbks.jsp?k=17
			...
			boolean flag = true;
			for (Element b : aDyxm) {
				if (b.attr("href").startsWith("print_ndsds_2014_mbks.jsp")) {
		...
		new CompanyDataDaoImpl().addData(siteHandler.params.getCompanyId(), "10062", list);
	...
	private Map<String, Object> parseLssb(Element tr) {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("10062001", StringUtils.StringFormat(tr.child(2).child(0).text()));
		map.put("10062002", StringUtils.StringFormat(tr.child(3).child(0).text()));
		map.put("10062003", StringUtils.StringFormat(tr.child(4).child(0).text()));
		map.put("serialNumber", "10062");
		return map;
	}
			...
			index6 = index4 + "001";
				for (int i = 1; i < tds1.size(); i++) {
					...
					if(k==personTrs1.size()-1){
						map.put(index6,td);
						index6 = String.valueOf((Long.parseLong(index6) + 11));
					}else {
						map.put(index6,td);
