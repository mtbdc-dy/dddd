...
/**
 *企业所得税和非居民企业所得税(包括季报和年报)--减免所得税优惠明细表 --A107040
 */
public class A107040_JmsdsyhmxbHandler implements GSModuleBase<GSSiteHandler> {
	...
	public SiteStatus start(GSSiteHandler siteHandler) throws SiteLoginFailedException {
		...
		for (int i =1; i < trElements.size(); i++) {
			...
			//http://100.0.0.1:8001/ctais2/wssb/sjcx/print_ndsds_2014_sejm.jsp?k=17
			...
			boolean flag = true;
			for (Element b : aDyxm) {
				if (b.attr("href").startsWith("print_ndsds_2014_sejm.jsp")) {
		...
		new CompanyDataDaoImpl().addData(siteHandler.params.getCompanyId(), "10064", list);
	...
	private Map<String, Object> parseLssb(Element tr) {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("10064001", StringUtils.StringFormat(tr.child(2).child(0).text()));
		map.put("10064002", StringUtils.StringFormat(tr.child(3).child(0).text()));
		map.put("10064003", StringUtils.StringFormat(tr.child(4).child(0).text()));
		map.put("serialNumber", "10064");
		return map;
	}
